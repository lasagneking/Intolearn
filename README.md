...
Personal-use prototype. Current build: **v7.7**.
...
**Content is supplied externally, not generated by Claude** — `knowledgeCards` in `app.js` (via `registerKnowledgeCards()`) currently holds one populated example (Wheat/gluten cereals) as a working proof of concept; the other 21 categories show "Coming soon" until their content is added in the same structure:
```js
{
  "CategoryName": {
...
```
Symptom icons come from a shared `SYMPTOM_ICONS` map (bloating, abdominalPain, diarrhoea, fatigue, headache, brainFog, nausea, vomiting, skin, joint, breathing) rather than one-off icons per card, so "Fatigue" always uses the same icon everywhere it appears — add to that map as new symptom concepts show up in future cards, rather than inventing a new icon per card.
